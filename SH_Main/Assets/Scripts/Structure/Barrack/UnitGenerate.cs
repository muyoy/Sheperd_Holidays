...
{
    public enum UnitType { Assasin =0,Sword , Javelin, Bow, Wizard }
            ...
            else
            {
                switch (unitType)
                {
                    case UnitType.Assasin:
                        if (GM.money < DBManager.instance.SheepDatas[0].cost) { break; }
                        tempSprite = Resources.Load("Unit/BarrackUnit/thief_Icon", typeof(Sprite)) as Sprite;
                        GM.money -= DBManager.instance.SheepDatas[0].cost;
                        barrackFunc.GenUnitQueue.Enqueue(new GenUnitInfo(DBManager.instance.SheepDatas[0], tempSprite));
                        break;
                    ...
                    case UnitType.Wizard:
                        if (GM.money < DBManager.instance.SheepDatas[4].cost) { break; }
                        tempSprite = Resources.Load("Unit/BarrackUnit/wand_Icon", typeof(Sprite)) as Sprite;
                        GM.money -= DBManager.instance.SheepDatas[4].cost;
                        barrackFunc.GenUnitQueue.Enqueue(new GenUnitInfo(DBManager.instance.SheepDatas[4], tempSprite));
                        break;
                }

                if(barrackFunc.GenUnitQueue.Count <= 0) { break; } else
                {
